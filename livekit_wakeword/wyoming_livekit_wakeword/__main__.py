...
from .handler import LiveKitWakeWordEventHandler
...
async def main() -> None:
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Path to directory with custom wake word models (.onnx)",
    )
    parser.add_argument(
        "--threshold",
        type=float,
    ...
    parser.add_argument(
        "--zeroconf",
        nargs="?",
        const="livekit-wakeword",
        help="Enable discovery over zeroconf with optional name (default: livekit-wakeword)",
    )
    ...
    for custom_model_dir_str in args.custom_model_dir:
        ...
        for model_path in custom_model_dir.glob("*.onnx"):
